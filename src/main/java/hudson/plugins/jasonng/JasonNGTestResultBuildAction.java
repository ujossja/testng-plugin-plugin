 ...
 * @since v1.0
 */
public class JasonNGTestResultBuildAction extends AbstractTestResultAction implements Serializable {
     ...
     */
    private transient Reference<JasonNGResult> testngResultRef;
    ...
    public JasonNGTestResultBuildAction(AbstractBuild<?, ?> build, JasonNGResult testngResults) {
        super(build);

        if (testngResults != null) {
            testngResults.setOwner(build);
            this.testngResultRef = new WeakReference<JasonNGResult>(testngResults);
    ...
    @Override
    public JasonNGResult getResult() {
        return getResult(super.owner);
    }

    public JasonNGResult getResult(AbstractBuild build) {
        if (testngResultRef == null) {
            testngResultRef = new WeakReference<JasonNGResult>(loadResults(build, null));
            return testngResultRef.get();
        }

        JasonNGResult tr = testngResultRef.get();
        if (tr == null) {
            testngResultRef = new WeakReference<JasonNGResult>(loadResults(build, null));
            return testngResultRef.get();
        } else {
    ...
    static JasonNGResult loadResults(AbstractBuild<?, ?> owner, PrintStream logger) {
        ...
        if (paths == null) {
            JasonNGResult tr = new JasonNGResult();
            tr.setOwner(owner);
            return tr;
        }

        ResultsParser parser = new ResultsParser(logger);
        JasonNGResult result = parser.parse(paths);
    ...
    public Object readResolve() {
        ...
        JasonNGResult testResults = getResult();
